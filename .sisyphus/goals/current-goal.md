...
- Batch 12 proof: Storybook typecheck passed, static Storybook build passed, CH5 coverage mapped `canvas` and `context`, and Chrome rendered desktop/mobile screenshots under `.sisyphus/evidence/storybook-coverage/batch-12/`. React Flow helper components (`connection`, `controls`, `edge`, `node`, `panel`, `toolbar`) remain unchecked because CH5 still reports direct local source gaps for those files.
