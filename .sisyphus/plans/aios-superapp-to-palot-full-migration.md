...
- [ ] T3. Port pane bus
  ...
  - Acceptance: [ ] file exists + typechecks; [ ] unit test for register/openFileInPane/openUrlInPane round-trip.
  - Evidence: `.sisyphus/evidence/aios-migration/task-3-pane-bus.txt`
  - Commit: `feat(shell): port pane bus into palot renderer`

- [ ] T4. Port pane routing helpers
...
- [ ] T5. Profiles / account context
  ...
  - Acceptance: [ ] profile atom persists; [ ] documented local-only vs synced decision.
...
- [ ] T6. Terminal/PTY backend seam
  ...
  - Acceptance: [ ] spawn shell, write, receive output, resize, kill via IPC in a node test/harness; [ ] disconnected/edge: kill nonexistent pid -> explicit error.
  - Evidence: `.sisyphus/evidence/aios-migration/task-6-pty.txt`
  - Commit: `feat(terminal): port PTY backend seam to electron main`

- [ ] T7. Files backend seam
  ...
  - Acceptance: [ ] read_dir_tree + git_status + read/write_text_file round-trip in harness; [ ] ENOENT path -> explicit error not crash.
  - Evidence: `.sisyphus/evidence/aios-migration/task-7-files.txt`
  - Commit: `feat(files): port files backend seam`

- [ ] T8. Office->PDF convert seam (Studio backend)
  ...
  - Acceptance: [ ] convert a sample .docx -> PDF path returned; [ ] missing soffice -> explicit unavailable error.
...
- [ ] T9. Terminal surface
  ...
  - Acceptance: [ ] open terminal surface, run `echo hi`, see output; [ ] resize keeps session; [ ] flag-off hides surface safely.
  - Evidence: `.sisyphus/evidence/aios-migration/task-9-terminal.txt`
  - Commit: `feat(terminal): ship terminal surface`

- [ ] T10. Files + file viewer surface
  ...
  - Acceptance: [ ] tree renders with git decorations; [ ] open file -> viewer; [ ] missing file -> explicit error state.
  - Evidence: `.sisyphus/evidence/aios-migration/task-10-files.txt`
  - Commit: `feat(files): ship files + viewer surface`

- [ ] T11. Editor surface (Monaco)
  ...
  - Acceptance: [ ] open a text file, edit, save via files seam; [ ] unsupported binary -> explicit message.
  - Evidence: `.sisyphus/evidence/aios-migration/task-11-editor.txt`
  - Commit: `feat(editor): ship monaco editor surface`

- [ ] T12. Studio / Office preview surface
  ...
  - Acceptance: [ ] preview a supported doc end-to-end; [ ] corrupt/missing -> clear error.
  - Evidence: `.sisyphus/evidence/aios-migration/task-12-studio.txt`
  - Commit: `feat(studio): ship office preview surface`

- [ ] T13. Browser hardening (existing surface)
  ...
  - Acceptance: [ ] load/nav/back/forward/reload; [ ] malformed URL -> graceful; [ ] state restores across relaunch.
...
- [ ] T14. Oracle roster surface + backend
  ...
  - Acceptance: [ ] roster lists tmux sessions (or empty when none); [ ] create/rename/kill round-trip; [ ] no-tmux -> empty, not error.
  - Evidence: `.sisyphus/evidence/aios-migration/task-14-oracle.txt`
  - Commit: `feat(oracle): port roster surface + tmux seam`

- [ ] T15. Voice dictation -> chat input
  ...
  - Acceptance: [ ] record short clip -> transcript inserted into chat input; [ ] denied mic / no whisper server -> explicit error, no silent drop.
  - Evidence: `.sisyphus/evidence/aios-migration/task-15-voice.txt`
  - Commit: `feat(voice): port dictation into chat input`

- [ ] T16. Bridges surface + backend
  ...
  - Acceptance: [ ] roster returns real connector records; [ ] unavailable connector -> explicit disconnected, not fake live.
...
- [ ] T17. Contacts / CRM surface + backend
  ...
  - Acceptance: [ ] list contacts/threads; [ ] save/delete contact round-trip; [ ] send_message path explicit on failure.
  - Evidence: `.sisyphus/evidence/aios-migration/task-17-crm.txt`
  - Commit: `feat(crm): port contacts/inbox surface + seam`

- [ ] T18. Plugins surface + backend
...
- [ ] T19. Claude Code compatibility surface
  ...
  - Acceptance: [ ] panel reflects real migration/compat state; [ ] unavailable -> explicit, no placeholder fiction.
...
- [ ] T20. Notes / Pulse / Memory hardening (existing shells -> real)
  ...
  - Acceptance: [ ] notes autosave/restore + inject-to-chat; [ ] pulse shows real metrics + empty/error states; [ ] memory list/search backed by real service + empty/error states.
...
- [ ] T21. Cross-cutting integration cleanup
  - Goal: fix only real issues revealed by combined runtime testing across all migrated surfaces.
  - Acceptance: [ ] no per-surface blocker remains; [ ] global gates green.
...
- [ ] T22. Excluded-domain documentation + reference scrub
  ...
  - Acceptance: [ ] zero palot references to superapp path; [ ] exclusions recorded in `docs/` or plan appendix.
...
- [ ] F1. Migration compliance audit — every PORT-list feature exists in palot with backend seam + surface + flag. Output APPROVE/REJECT.
- [ ] F2. Code quality — `bun run lint` + `bun run check-types` + `bun run build`; scan for `as any`, `@ts-ignore`, renderer Node imports, demo/mock residue. Output APPROVE/REJECT.
- [ ] F3. Runtime QA sweep — execute every task QA scenario; integrated boot of Palot exercising each migrated surface. Output APPROVE/REJECT.
- [ ] F4. Superapp retirement audit — grep palot for any remaining dependency on `aios-superapp`; confirm every non-excluded source surface/command is represented; produce the definitive "safe to delete `~/Workspaces/aios-superapp`" verdict with per-feature mapping. Output APPROVE/REJECT.

---

## Success Criteria <!-- oc:id=sec_af -->
- [ ] Terminal, Files, Editor, Oracle, Voice, Bridges, CRM, Studio, Plugins, Claude-compat all exist in Palot as real surfaces with real backend seams.
- [ ] browser/notes/pulse/memory hardened to production (not proof shells).
- [ ] No palot code references `~/Workspaces/aios-superapp`.
- [ ] Motion/Database/Monitor/Device/IdleDashboard explicitly excluded and documented.
- [ ] Global gates pass: lint, check-types, build, boot.
- [ ] F4 retirement audit = APPROVE (superapp can be deleted).

---

## Final Checklist
- [ ] All PORT-list features migrated with evidence.
- [ ] Evidence files exist for every task.
- [ ] F1-F4 all APPROVE.
- [ ] Retirement verdict recorded; superapp repo no longer referenced.
