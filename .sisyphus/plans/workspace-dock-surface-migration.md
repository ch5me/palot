...
- **Status:** ✅ COMPLETE on `feat/workspace-dock-surface-migration` (30 commits, pushed, PR-ready). All 16 surfaces migrated to V2 plugins; feature flags removed; dock is the sole session UI (proven by default in the web build); deep-link focus wired; Storybook regression proof; **conservative eviction landed + proven**; **layout persistence landed** (surface→zone arrangement persists across reloads, session-agnostic via surfaceType, restore safe on first-load/parse-failure, Storybook round-trip test). PR: ch5me/palot#1. ALL plan phases complete. tsgo + biome + full electron-vite build all green. Resource-specific controllers were dropped as unnecessary (panels own their editor/xterm instances and self-manage resize). **Remaining = decision-gated / outward-facing only:** an Electron click-through and merging the PR. (PR opened: ch5me/palot#1. The `@ch5me/agent-ui-web` terminal-in-web quirk is FIXED at source — ch5-packages commit `8a46993`, `resolveAnsi()` unwraps the CJS interop double-nesting; the Terminal React crash is gone, it now correctly shows "PTY only in Electron" in web. Layout persistence landed.) NOTE: the long-running web dev server holds a stale HMR chunk referencing the deleted `workspaceDockEnabledAtom` (throws only on one cached route) — not a code bug (zero source refs, prod build clean); a dev-server restart clears it.
...
- **(D) Terminal panel renders `undefined` in the WEB build** ("Element type is invalid … TerminalContent"). Root cause is PRE-EXISTING: `terminal-panel.tsx` imports `Terminal*` from `@ch5me/agent-ui-web` — byte-identical to the pre-migration file (`git show 344496bc7:…/side-panel/terminal-panel.tsx`). The export resolves to `undefined` only in the web bundle; Electron is unaffected (imports unchanged, terminal worked there). The dock merely now *exposes* terminal in web. Fix belongs in `@ch5me/agent-ui-web`/web bundling, not this migration. Caught gracefully by the boundary.
  ...
  **✅ RESOLVED at source — ch5-packages `8a46993`** (`resolveAnsi()` in agent-ui-web's terminal.tsx unwraps the CJS interop double-nesting; Terminal React crash gone, web now shows the correct "PTY only in Electron" message). Original investigation kept for history:
  **Investigated 2026-06-16 (root cause):** `@ch5me/agent-ui-web`'s `TerminalContent` renders `ansi-to-react` (CJS: `exports.__esModule = true; exports.default = Ansi`). When Vite's esbuild pre-bundler inlines `ansi-to-react` into the `@ch5me_agent-ui-web` optimized chunk, it applies node-mode interop (`__toESM(require(...), 1)`) and binds the whole CJS exports object as `.default` instead of the `Ansi` function → React gets an object as an element type. Electron's Rollup build unwraps `__esModule` correctly, so only the Vite **dev web** path breaks. Note `@ch5me/agent-ui-web` imports its OWN nested `ansi-to-react` (`ch5-packages/node_modules/.pnpm/ansi-to-react@6.2.6.../`), a different physical path than palot's root copy. Tried `optimizeDeps.include: ["ansi-to-react"]` (wrong copy — no effect) and the nested form `["@ch5me/agent-ui-web > ansi-to-react"]` (also no effect — `>` likely doesn't match the deep pnpm path). **Recommended real fixes (untried, for whoever owns it):** (a) try `optimizeDeps.exclude: ["@ch5me/agent-ui-web"]` so Vite serves it as source ESM (Vite's per-import interop handles `__esModule` default correctly) — risk: may affect the package's other dev bundling; (b) fix it at the source in `@ch5me/agent-ui-web`'s build so it ships proper ESM and doesn't leave the raw CJS `ansi-to-react` to be mis-interoped. DEV-ONLY; production Electron ships fine. Config experiments were reverted to keep `vite.web.config.ts` clean.
