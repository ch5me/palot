...
> Update (2026-06-09): Chris requested full coverage — the former skips are now ported too, with marketing one-offs isolated in a `src/components/marketing/` section.

| # | Component | Status | Decision | Complexity | New deps | Reason |
|---|---|---|---|---|---|---|
| 1 | `3d-book` | missing | **ported** → `src/components/marketing/3d-book.tsx` | medium | — | Marketing flourish (3D fanning book, forces literal `light` class); no use in a chat/agent desktop or general app surface. |
...
| 3 | `bento-card` | missing | **ported** → `src/components/marketing/bento-card.tsx` | high | — | Marketing-page bento with mock workspace preview; one-off landing-page content, not a product-app primitive. |
...
| 5 | `bucket` | missing | **ported** → `src/components/marketing/bucket.tsx` | high | — | Figma-exported SVG marketing gag (chips tossed into a bucket); zero app utility, hard-coded art. |
...
| 8 | `discover-button` | missing | **ported** → `src/components/marketing/discover-button.tsx` | low | — | One-off marketing like/discover expansion, light-only raw-gray palette; duplicative with status/delete button morph patterns being ported. |
...
| 11 | `empty-testimonial` | partial | **ported** → `src/components/marketing/empty-testimonial.tsx` | low | — | Testimonial-specific marketing empty state; we already have a generic Empty component. Folder-open hover gag too site-specific. |
...
| 13 | `feature-carousel` | partial | **ported** → `src/components/marketing/feature-carousel.tsx` | medium | — | Marketing feature carousel with Unsplash imagery + hardcoded brand blue; we already have embla Carousel for product needs. |
...
| 15 | `folder-interaction` | missing | **ported** → `src/components/marketing/folder-interaction.tsx` | medium | — | Figma-exported SVG folder with fully hardcoded lavender palette; pure marketing eye-candy requiring a total retheme for near-zero app value. |
...
| 18 | `magnified-bento` | missing | **ported** → `src/components/marketing/magnified-bento.tsx` | high | — | Draggable magnifier lens over marquee chips — marketing bento one-off with hardcoded gray ramp; no app surface needs it. |
...
| 22 | `shake-testimonial-card` | missing | **ported** → `src/components/marketing/shake-testimonial-card.tsx` | low | — | Marketing testimonial card with hardcoded pastel sticky-note hexes; site-specific, light-only, no app use. |
| 23 | `smooth-dropdown` | have | **ported** → `src/components/animate/smooth-dropdown.tsx` | medium | `react-use-measure` | Ported as a standalone animated dropdown (Chris requested full coverage); the measured-height spring + layoutId hover highlight may still be folded into the accessible base-ui dropdown-menu.tsx later. |
