...
export async function activateBrowserLaneCdpTab(cdpEndpoint: string, tabId: string): Promise<void> {
	const endpoint = cdpEndpoint.replace(/\/$/, "")
...
async function resolvePageTarget(cdpEndpoint: string, tabId?: string | null): Promise<CdpTarget> {
	...
	if (!page) {
		throw new Error(
			tabId ? `Browser lane tab ${tabId} not found` : "Browser lane has no navigable page target",
		)
	}
