...
export function ProjectManager() {
	...
	const handleProjectManagerLaunch = useCallback(
		async (_promptText: string, _files?: FileAttachment[]) => {
			if (!selectedDirectory) return
			persistProjectModel()
			clearDraft()
			setError(
				`Project Manager shell ready for ${selectedProject?.name ?? "project"}. Prompt routing and PM session orchestration still need wiring.`,
			)
		},
		[clearDraft, persistProjectModel, selectedDirectory, selectedProject?.name],
	)
						...
						{SUGGESTIONS.map((suggestion) => {
							...
							return (
								<button
									...
									className="group/card flex flex-col gap-3 rounded-xl border border-border/50 bg-background/40 p-4 text-left backdrop-blur-sm transition-colors hover:border-muted-foreground/30 hover:bg-background/60 disabled:opacity-50"
								>
