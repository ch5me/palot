...
async function probeLaneHealth(
	lane: Pick<
		BrowserLane,
		"runtimeOwnership" | "surfaceKind" | "targetUrl" | "streamBackendUrl" | "cdpEndpoint" | "profilePath"
	>,
	profileResetAt: number | null,
): Promise<BrowserLaneHealth> {
	const surfaceUrl = getBrowserLaneSurfaceUrl(lane)
	const directIframe = isDirectIframeBrowserLane(lane)
...
export async function ensureBrowserLane(
	...
	return await withLaneLock(id, async () => {
		const ensured = await ensureBrowserLanePrepared(id)
		return options.autostart ? await startBrowserLaneLocked(id, ensured) : ensured
	})
}
...
async function ensureBrowserLanePrepared(id: string): Promise<BrowserLane> {
	...
	if (!lane) throw new Error(`Browser lane ${id} not found`)
	if (isAttachedBrowserLane(lane)) {
		setLaneHealth(id, await probeLaneHealth(lane, getLaneState(id).profileResetAt))
...
async function startBrowserLaneLocked(id: string, preparedLane?: BrowserLane): Promise<BrowserLane> {
	const lane = preparedLane ?? (await ensureBrowserLanePrepared(id))
	const state = getLaneState(id)
	...
	const probe = await waitForBrowserLaneEndpoints({
		streamUrl: lane.streamBackendUrl,
		cdpUrl: lane.cdpEndpoint,
...
export async function refreshBrowserLaneHealth(id: string): Promise<BrowserLaneHealth> {
	...
	if (!lane) throw new Error(`Browser lane ${id} not found`)
	const state = getLaneState(id)
	const health = await probeLaneHealth(lane, state.profileResetAt)
