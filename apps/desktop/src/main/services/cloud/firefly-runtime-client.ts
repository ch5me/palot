 ...
 *
 * The apiUrl defaults to `https://api.elf.dance` and can be overridden with
 * `FIREFLY_API_URL` (production) or `VITE_FIREFLY_API_URL` (build-time).
 *
...
const DEFAULT_FIREFLY_API_URL = "https://api.elf.dance"
...
function resolveApiUrl(override?: string): string {
  return (
    ...
    process.env.VITE_FIREFLY_API_URL ??
    DEFAULT_FIREFLY_API_URL
  ).replace(/\/+$/, "")
}
