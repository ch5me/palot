...
contextBridge.exposeInMainWorld("elf", {
	...
	palot: {
		...
		openLogicalPanel: (route: import("./api").PalotLogicalPanelRoute) =>
			ipcRenderer.invoke("palot:open-logical-panel", route),
		...
		onOpenSidePanel: (callback: (payload: import("./api").PalotLogicalPanelRoute) => void) => {
			const listener = (_event: unknown, payload: import("./api").PalotLogicalPanelRoute) => callback(payload)
