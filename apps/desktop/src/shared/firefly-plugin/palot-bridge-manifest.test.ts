...
describe("palotBridgeManifest", () => {
	...
	test("declares all 14 current product + discovery tools", () => {
		const parsed = parsePluginManifest(palotBridgeManifest)
		expect(parsed.contributes.tools).toHaveLength(14)
		...
		expect(names).toContain("plugin.firefly.built-in.palot-bridge.open_logical_panel")
	...
	test("every tool that performs a side-effect declares the matching capability", () => {
		...
		expect(byName("open_logical_panel")?.requires).toContain("host:bridge.ui-state-write")
