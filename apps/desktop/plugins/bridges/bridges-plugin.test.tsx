/**
 * Slice-proof tests for the Bridges migration (plan §6 criteria):
 *  1. registry row deleted — grep-proof on the registry source;
 *  2. the Bridges tab derives solely from the catalog projection;
...
import {
	BRIDGES_PANEL_PROJECTED_ID,
	BRIDGES_PLUGIN_ID,
	BRIDGES_TOOL_OPEN_ID,
	BRIDGES_TOOL_STATE_ID,
...
const FLAG_SOURCE = path.join(
	import.meta.dir,
	"../../src/renderer/atoms/feature-flags.ts",
)

function fakeBridgesDeps() {
	const opened: string[] = []
	let enabled = true
	return {
		opened,
		deps: {
			openSidePanel: async (tab: "bridges") => {
				opened.push(tab)
			},
			getSidePanelState: () => ({
				open: true,
				activeTab: "bridges",
				availableTabs: ["review", "bridges", "files"],
			}),
...
describe("criterion 1 — registry row deleted (grep-proof)", () => {
	test("firefly-surface-registry.tsx has no bridges surface row", () => {
		const source = fs.readFileSync(REGISTRY_SOURCE, "utf8")
		expect(source).not.toContain('id: "bridges"')
		expect(source).not.toContain("BridgesPanel")
		expect(source).not.toContain("bridgesSurfaceEnabledAtom")
	})

	test("the legacy panel file is gone from components/side-panel", () => {
		const legacyPath = path.join(
			import.meta.dir,
			"../../src/renderer/components/side-panel/bridges-panel.tsx",
		)
		expect(fs.existsSync(legacyPath)).toBe(false)
	})

	test("feature-flags.ts has no bridgesSurfaceEnabledAtom", () => {
		const source = fs.readFileSync(FLAG_SOURCE, "utf8")
		expect(source).not.toContain("bridgesSurfaceEnabledAtom")
		expect(source).not.toContain("toggleBridgesSurfaceAtom")
	})
})

describe("criterion 2 — Bridges serves from the catalog projection", () => {
	test("manifest parses and derives a descriptor on the canonical path", () => {
		const manifest = parsePluginManifest(bridgesPluginManifest)
		const descriptor = derivePluginDescriptor(manifest, { appVersion: "0.11.0" })
		expect(descriptor.normalizedId).toBe(BRIDGES_PLUGIN_ID)
		expect(descriptor.panels).toHaveLength(1)
		expect(descriptor.tools.map((t) => t.id)).toEqual([BRIDGES_TOOL_OPEN_ID, BRIDGES_TOOL_STATE_ID])
	})

	test("catalog projects the bridges side-panel tab with stable identity", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const panel = catalog.projections.panels.find((p) => p.pluginId === BRIDGES_PLUGIN_ID)
		expect(panel).toBeDefined()
		expect(panel?.projectedId).toBe(BRIDGES_PANEL_PROJECTED_ID)
		expect(panel?.availability.available).toBe(true)

		const descriptor = catalogPanelToTabDescriptor(panel!)
		expect(descriptor).not.toBeNull()
		expect(descriptor?.id).toBe("bridges")
		// criterion 6: persistence + telemetry identity survive the cutover.
		expect(descriptor?.persistenceKey).toBe("side-panel.bridges")
		expect(descriptor?.telemetryNamespace).toBe("firefly.surface.bridges")
		expect(descriptor?.renderMode).toBe("host-reconciler")
	})

	test("bridges tools project into the bridge tool surface (OpenCode visibility)", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const descriptor = catalog.descriptors.find((d) => d.normalizedId === BRIDGES_PLUGIN_ID)
		const projected = projectBridgeToolDefinitions(descriptor!)
		expect(projected.map((t) => t.id)).toContain(BRIDGES_TOOL_OPEN_ID)
		expect(projected.map((t) => t.id)).toContain(BRIDGES_TOOL_STATE_ID)
	})
})

describe("criterion 3 — paired tools dispatch with typed envelopes", () => {
	test("bridges.open opens the side panel", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeBridgesDeps()
		registerBridgesHostHandlers(deps)
		const result = await invokePluginTool({
			pluginId: BRIDGES_PLUGIN_ID,
			toolId: BRIDGES_TOOL_OPEN_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
		expect(opened).toEqual(["bridges"])
		expect((result.data as { tab: string }).tab).toBe("bridges")
	})

	test("bridges.state returns typed surface state", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		registerBridgesHostHandlers(fakeBridgesDeps().deps)
		const result = await invokePluginTool({
			pluginId: BRIDGES_PLUGIN_ID,
			toolId: BRIDGES_TOOL_STATE_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
	...
	test("unknown tool and invalid args produce typed failures, not throws", async () => {
		...
		const unknown = await invokePluginTool({
			pluginId: BRIDGES_PLUGIN_ID,
			toolId: "plugin.firefly.built-in.surface.bridges.ghost",
			args: {},
		})
	...
	test("open-bridges / toggle-bridges commands dispatch through the V2 command path", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeBridgesDeps()
...
describe("criterion 4 — disable/enable round-trip via host lifecycle", () => {
	test("disable flips catalog status + projection availability; tools refuse; enable restores", async () => {
		...
		setPluginEnabled(BRIDGES_PLUGIN_ID, false)
		const disabledCatalog = getPluginCatalog()
		const entry = disabledCatalog.entries.find((e) => e.pluginId === BRIDGES_PLUGIN_ID)
		expect(entry?.status).toBe("disabled")
		const panel = disabledCatalog.projections.panels.find((p) => p.pluginId === BRIDGES_PLUGIN_ID)
		...
		const commands = disabledCatalog.projections.commands.filter(
			(c) => c.pluginId === BRIDGES_PLUGIN_ID,
		)
		...
		setPluginEnabled(BRIDGES_PLUGIN_ID, true)
		const restored = getPluginCatalog()
		expect(
			restored.projections.panels.find((p) => p.pluginId === BRIDGES_PLUGIN_ID)?.availability
				.available,
		).toBe(true)
	})
})
...
describe("criterion 5 — UI-crash quarantine drill", () => {
	test("3 panel crashes quarantine the plugin; release restores it", async () => {
		...
		reportPluginPanelCrash(BRIDGES_PLUGIN_ID, "drill crash 1")
		reportPluginPanelCrash(BRIDGES_PLUGIN_ID, "drill crash 2")
		const third = reportPluginPanelCrash(BRIDGES_PLUGIN_ID, "drill crash 3")
		expect(third.quarantined).toBe(true)

		const catalog = getPluginCatalog()
		const entry = catalog.entries.find((e) => e.pluginId === BRIDGES_PLUGIN_ID)
		...
		releasePluginQuarantine(BRIDGES_PLUGIN_ID, "drill release")
		const restored = getPluginCatalog()
		expect(restored.entries.find((e) => e.pluginId === BRIDGES_PLUGIN_ID)?.status).toBe("validated")
	})
})
...
describe("criterion 7 — legacy flag migration", () => {
	...
	test("flag=false migrates to a host-side disable, once", async () => {
		const { io, enabledCalls, store } = migrationIo({ "elf:bridgesSurfaceEnabled": "false" })
		const first = await migrateSurfaceFlag(migration, io)
		expect(first.action).toBe("migrated-disabled")
		expect(enabledCalls).toEqual([{ pluginId: BRIDGES_PLUGIN_ID, enabled: false }])
		expect(store.has("elf:bridgesSurfaceEnabled")).toBe(false)
	...
	test("flag=true needs no host write (enabled is the default)", async () => {
		const { io, enabledCalls } = migrationIo({ "elf:bridgesSurfaceEnabled": "true" })
