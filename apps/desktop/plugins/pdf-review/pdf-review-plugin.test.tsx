/**
 * Slice-proof tests for the PDF Review migration (plan §6 criteria):
 *  1. registry row deleted — grep-proof on the registry source;
 *  2. the PDF Review tab derives solely from the catalog projection;
...
import {
	PDF_REVIEW_PANEL_PROJECTED_ID,
	PDF_REVIEW_PLUGIN_ID,
	PDF_REVIEW_TOOL_OPEN_ID,
	PDF_REVIEW_TOOL_STATE_ID,
...
function fakePdfReviewDeps() {
	const opened: string[] = []
	let enabled = false
	return {
		opened,
		deps: {
			openSidePanel: async (tab: "pdf-review") => {
				opened.push(tab)
			},
			getSidePanelState: () => ({
				open: true,
				activeTab: "pdf-review",
				availableTabs: ["review", "notes", "pdf-review"],
			}),
...
describe("criterion 1 — registry row deleted (grep-proof)", () => {
	test("firefly-surface-registry.tsx has no pdf-review surface row", () => {
		const source = fs.readFileSync(REGISTRY_SOURCE, "utf8")
		expect(source).not.toContain('id: "pdf-review"')
		expect(source).not.toContain("PdfReviewPanel")
		expect(source).not.toContain("pdfReviewSurfaceEnabledAtom")
	})

	test("the legacy panel file is gone from components/side-panel", () => {
		const legacyPath = path.join(
			import.meta.dir,
			"../../src/renderer/components/side-panel/pdf-review-panel.tsx",
		)
		expect(fs.existsSync(legacyPath)).toBe(false)
	})
})

describe("criterion 2 — PDF Review serves from the catalog projection", () => {
	test("manifest parses and derives a descriptor on the canonical path", () => {
		const manifest = parsePluginManifest(pdfReviewPluginManifest)
		const descriptor = derivePluginDescriptor(manifest, { appVersion: "0.11.0" })
		expect(descriptor.normalizedId).toBe(PDF_REVIEW_PLUGIN_ID)
		expect(descriptor.panels).toHaveLength(1)
		expect(descriptor.tools.map((t) => t.id)).toEqual([PDF_REVIEW_TOOL_OPEN_ID, PDF_REVIEW_TOOL_STATE_ID])
	})

	test("catalog projects the pdf-review side-panel tab with stable identity", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const panel = catalog.projections.panels.find((p) => p.pluginId === PDF_REVIEW_PLUGIN_ID)
		expect(panel).toBeDefined()
		expect(panel?.projectedId).toBe(PDF_REVIEW_PANEL_PROJECTED_ID)
		expect(panel?.availability.available).toBe(true)

		const descriptor = catalogPanelToTabDescriptor(panel!)
		expect(descriptor).not.toBeNull()
		expect(descriptor?.id).toBe("pdf-review")
		// criterion 6: persistence + telemetry identity survive the cutover (byte-match old row).
		expect(descriptor?.persistenceKey).toBe("side-panel.pdf-review")
		expect(descriptor?.telemetryNamespace).toBe("firefly.surface.pdf-review")
		expect(descriptor?.renderMode).toBe("host-reconciler")
	})

	test("pdf-review tools project into the bridge tool surface (OpenCode visibility)", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const descriptor = catalog.descriptors.find((d) => d.normalizedId === PDF_REVIEW_PLUGIN_ID)
		const projected = projectBridgeToolDefinitions(descriptor!)
		expect(projected.map((t) => t.id)).toContain(PDF_REVIEW_TOOL_OPEN_ID)
		expect(projected.map((t) => t.id)).toContain(PDF_REVIEW_TOOL_STATE_ID)
	})
})

describe("criterion 3 — paired tools dispatch with typed envelopes", () => {
	test("pdf-review.open opens the side panel", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakePdfReviewDeps()
		registerPdfReviewHostHandlers(deps)
		const result = await invokePluginTool({
			pluginId: PDF_REVIEW_PLUGIN_ID,
			toolId: PDF_REVIEW_TOOL_OPEN_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
		expect(opened).toEqual(["pdf-review"])
		expect((result.data as { tab: string }).tab).toBe("pdf-review")
	})

	test("pdf-review.state returns typed surface state", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		registerPdfReviewHostHandlers(fakePdfReviewDeps().deps)
		const result = await invokePluginTool({
			pluginId: PDF_REVIEW_PLUGIN_ID,
			toolId: PDF_REVIEW_TOOL_STATE_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
	...
	test("unknown tool and invalid args produce typed failures, not throws", async () => {
		...
		const unknown = await invokePluginTool({
			pluginId: PDF_REVIEW_PLUGIN_ID,
			toolId: "plugin.firefly.built-in.surface.pdf-review.ghost",
			args: {},
		})
	...
	test("open-pdf-review / toggle-pdf-review commands dispatch through the V2 command path", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakePdfReviewDeps()
		...
		const toggle = await invokePluginCommand({
			pluginId: PDF_REVIEW_PLUGIN_ID,
			commandId: "toggle-pdf-review",
			args: {},
		})
		expect(toggle.status).toBe("completed")
		// After reset, the plugin is validated (enabled). Toggling once disables it.
...
describe("criterion 4 — disable/enable round-trip via host lifecycle", () => {
	test("disable flips catalog status + projection availability; tools refuse; enable restores", async () => {
		...
		setPluginEnabled(PDF_REVIEW_PLUGIN_ID, false)
		const disabledCatalog = getPluginCatalog()
		const entry = disabledCatalog.entries.find((e) => e.pluginId === PDF_REVIEW_PLUGIN_ID)
		expect(entry?.status).toBe("disabled")
		const panel = disabledCatalog.projections.panels.find((p) => p.pluginId === PDF_REVIEW_PLUGIN_ID)
		...
		const commands = disabledCatalog.projections.commands.filter(
			(c) => c.pluginId === PDF_REVIEW_PLUGIN_ID,
		)
		...
		setPluginEnabled(PDF_REVIEW_PLUGIN_ID, true)
		const restored = getPluginCatalog()
		expect(
			restored.projections.panels.find((p) => p.pluginId === PDF_REVIEW_PLUGIN_ID)?.availability
				.available,
		).toBe(true)
	})
})
...
describe("criterion 5 — UI-crash quarantine drill", () => {
	test("3 panel crashes quarantine the plugin; release restores it", async () => {
		...
		reportPluginPanelCrash(PDF_REVIEW_PLUGIN_ID, "drill crash 1")
		reportPluginPanelCrash(PDF_REVIEW_PLUGIN_ID, "drill crash 2")
		const third = reportPluginPanelCrash(PDF_REVIEW_PLUGIN_ID, "drill crash 3")
		expect(third.quarantined).toBe(true)

		const catalog = getPluginCatalog()
		const entry = catalog.entries.find((e) => e.pluginId === PDF_REVIEW_PLUGIN_ID)
		...
		releasePluginQuarantine(PDF_REVIEW_PLUGIN_ID, "drill release")
		const restored = getPluginCatalog()
		expect(restored.entries.find((e) => e.pluginId === PDF_REVIEW_PLUGIN_ID)?.status).toBe("validated")
	})
})
...
describe("criterion 7 — legacy flag migration", () => {
	...
	test("flag=false migrates to a host-side disable, once", async () => {
		const { io, enabledCalls, store } = migrationIo({ "elf:pdfReviewSurfaceEnabled": "false" })
		const first = await migrateSurfaceFlag(migration, io)
		expect(first.action).toBe("migrated-disabled")
		expect(enabledCalls).toEqual([{ pluginId: PDF_REVIEW_PLUGIN_ID, enabled: false }])
		expect(store.has("elf:pdfReviewSurfaceEnabled")).toBe(false)
	...
	test("flag=true needs no host write (enabled is the default)", async () => {
		const { io, enabledCalls } = migrationIo({ "elf:pdfReviewSurfaceEnabled": "true" })
	...
	test("SURFACE_FLAG_MIGRATIONS includes pdf-review row with correct key", async () => {
		const { SURFACE_FLAG_MIGRATIONS } = await import("../../src/renderer/firefly-plugin-flag-migration")
		const row = SURFACE_FLAG_MIGRATIONS.find((m) => m.pluginId === PDF_REVIEW_PLUGIN_ID)
		expect(row).toBeDefined()
		expect(row?.legacyStorageKey).toBe("elf:pdfReviewSurfaceEnabled")
	})
})
