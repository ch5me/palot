/**
 * Slice-proof tests for the Voice surface migration (plan §6 criteria):
 *  1. V1 registry row deleted — grep-proof on the registry source;
 *  2. the Voice tab derives solely from the catalog projection;
...
function fakeVoiceDeps() {
	...
	return {
		opened,
		deps: {
			openSidePanel: async (tab: "voice") => {
				opened.push(tab)
			},
			getSidePanelState: () => ({
				open: true,
				activeTab: "voice",
				availableTabs: ["review", "voice", "files"],
			}),
...
describe("criterion 1 — registry row deleted (grep-proof)", () => {
	test("firefly-surface-registry.tsx has no voice surface row", () => {
		const source = fs.readFileSync(REGISTRY_SOURCE, "utf8")
		expect(source).not.toContain('id: "voice"')
		expect(source).not.toContain("VoicePanel")
		expect(source).not.toContain("voiceSurfaceEnabledAtom")
	})

	test("the legacy panel file is gone from components/side-panel", () => {
		const legacyPath = path.join(
			import.meta.dir,
			"../../src/renderer/components/side-panel/voice-panel.tsx",
		)
		expect(fs.existsSync(legacyPath)).toBe(false)
	})
})

describe("criterion 2 — Voice serves from the catalog projection", () => {
	test("manifest parses and derives a descriptor on the canonical path", () => {
		const manifest = parsePluginManifest(voicePluginManifest)
		const descriptor = derivePluginDescriptor(manifest, { appVersion: "0.11.0" })
		expect(descriptor.normalizedId).toBe(VOICE_PLUGIN_ID)
		expect(descriptor.panels).toHaveLength(1)
		expect(descriptor.tools.map((t) => t.id)).toEqual([VOICE_TOOL_OPEN_ID, VOICE_TOOL_STATE_ID])
	})

	test("catalog projects the voice side-panel tab with stable identity", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const panel = catalog.projections.panels.find((p) => p.pluginId === VOICE_PLUGIN_ID)
		expect(panel).toBeDefined()
		expect(panel?.projectedId).toBe(VOICE_PANEL_PROJECTED_ID)
		expect(panel?.availability.available).toBe(true)

		const descriptor = catalogPanelToTabDescriptor(panel!)
		expect(descriptor).not.toBeNull()
		expect(descriptor?.id).toBe("voice")
		// criterion 6: persistence + telemetry identity survive the cutover.
		expect(descriptor?.persistenceKey).toBe("side-panel.voice")
		expect(descriptor?.telemetryNamespace).toBe("firefly.surface.voice")
		expect(descriptor?.renderMode).toBe("host-reconciler")
	})

	test("voice tools project into the bridge tool surface (OpenCode visibility)", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const descriptor = catalog.descriptors.find((d) => d.normalizedId === VOICE_PLUGIN_ID)
		const projected = projectBridgeToolDefinitions(descriptor!)
		expect(projected.map((t) => t.id)).toContain(VOICE_TOOL_OPEN_ID)
		expect(projected.map((t) => t.id)).toContain(VOICE_TOOL_STATE_ID)
	})
})

describe("criterion 3 — paired tools dispatch with typed envelopes", () => {
	test("voice.open opens the side panel", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeVoiceDeps()
		registerVoiceHostHandlers(deps)
		const result = await invokePluginTool({
			pluginId: VOICE_PLUGIN_ID,
			toolId: VOICE_TOOL_OPEN_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
		expect(opened).toEqual(["voice"])
		expect((result.data as { tab: string }).tab).toBe("voice")
	})

	test("voice.state returns typed surface state", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		registerVoiceHostHandlers(fakeVoiceDeps().deps)
		const result = await invokePluginTool({
			pluginId: VOICE_PLUGIN_ID,
			toolId: VOICE_TOOL_STATE_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
	...
	test("unknown tool and invalid args produce typed failures, not throws", async () => {
		...
		const unknown = await invokePluginTool({
			pluginId: VOICE_PLUGIN_ID,
			toolId: "plugin.firefly.built-in.surface.voice.ghost",
			args: {},
		})
	...
	test("open-voice / toggle-voice commands dispatch through the V2 command path", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeVoiceDeps()
...
describe("criterion 4 — disable/enable round-trip via host lifecycle", () => {
	test("disable flips catalog status + projection availability; tools refuse; enable restores", async () => {
		...
		setPluginEnabled(VOICE_PLUGIN_ID, false)
		const disabledCatalog = getPluginCatalog()
		const entry = disabledCatalog.entries.find((e) => e.pluginId === VOICE_PLUGIN_ID)
		expect(entry?.status).toBe("disabled")
		const panel = disabledCatalog.projections.panels.find((p) => p.pluginId === VOICE_PLUGIN_ID)
		...
		setPluginEnabled(VOICE_PLUGIN_ID, true)
		const restored = getPluginCatalog()
		expect(
			restored.projections.panels.find((p) => p.pluginId === VOICE_PLUGIN_ID)?.availability
				.available,
		).toBe(true)
	})
})
...
describe("criterion 5 — UI-crash quarantine drill", () => {
	test("3 panel crashes quarantine the plugin; release restores it", async () => {
		...
		reportPluginPanelCrash(VOICE_PLUGIN_ID, "drill crash 1")
		reportPluginPanelCrash(VOICE_PLUGIN_ID, "drill crash 2")
		const third = reportPluginPanelCrash(VOICE_PLUGIN_ID, "drill crash 3")
		expect(third.quarantined).toBe(true)

		const catalog = getPluginCatalog()
		const entry = catalog.entries.find((e) => e.pluginId === VOICE_PLUGIN_ID)
		...
		releasePluginQuarantine(VOICE_PLUGIN_ID, "drill release")
		const restored = getPluginCatalog()
		expect(restored.entries.find((e) => e.pluginId === VOICE_PLUGIN_ID)?.status).toBe("validated")
	})
})
...
describe("criterion 7 — legacy flag migration", () => {
	...
	test("flag=false migrates to a host-side disable, once", async () => {
		const { io, enabledCalls, store } = migrationIo({ "elf:voiceSurfaceEnabled": "false" })
		const first = await migrateSurfaceFlag(migration, io)
		expect(first.action).toBe("migrated-disabled")
		expect(enabledCalls).toEqual([{ pluginId: VOICE_PLUGIN_ID, enabled: false }])
		expect(store.has("elf:voiceSurfaceEnabled")).toBe(false)
	...
	test("flag=true needs no host write (enabled is the default)", async () => {
		const { io, enabledCalls } = migrationIo({ "elf:voiceSurfaceEnabled": "true" })
