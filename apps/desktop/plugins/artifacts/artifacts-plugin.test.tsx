/**
 * Slice proof tests for the Artifacts migration (per-surface migration checklist §6):
 *  1. registry row deleted — grep-proof on the registry source;
 *  2. the Artifacts tab derives solely from the catalog projection;
...
import {
	ARTIFACTS_PANEL_PROJECTED_ID,
	ARTIFACTS_PLUGIN_ID,
	ARTIFACTS_TOOL_OPEN_ID,
	ARTIFACTS_TOOL_STATE_ID,
...
function fakeArtifactsDeps() {
	const opened: string[] = []
	let enabled = true
	return {
		opened,
		deps: {
			openSidePanel: async (tab: "artifacts") => {
				opened.push(tab)
			},
			getSidePanelState: () => ({
				open: true,
				activeTab: "artifacts",
				availableTabs: ["review", "notes", "files", "artifacts"],
			}),
...
describe("criterion 1 — registry row deleted (grep-proof)", () => {
	test("firefly-surface-registry.tsx has no artifacts surface row", () => {
		const source = fs.readFileSync(REGISTRY_SOURCE, "utf8")
		expect(source).not.toContain('id: "artifacts"')
		expect(source).not.toContain("ArtifactsPanel")
		expect(source).not.toContain("artifactsSurfaceEnabledAtom")
	})

	test("the legacy panel file is gone from components/side-panel", () => {
		const legacyPath = path.join(
			import.meta.dir,
			"../../src/renderer/components/side-panel/artifacts-panel.tsx",
		)
		expect(fs.existsSync(legacyPath)).toBe(false)
	})
})

describe("criterion 2 — Artifacts serves from the catalog projection", () => {
	test("manifest parses and derives a descriptor on the canonical path", () => {
		const manifest = parsePluginManifest(artifactsPluginManifest)
		const descriptor = derivePluginDescriptor(manifest, { appVersion: "0.11.0" })
		expect(descriptor.normalizedId).toBe(ARTIFACTS_PLUGIN_ID)
		expect(descriptor.panels).toHaveLength(1)
		expect(descriptor.tools.map((t) => t.id)).toEqual([ARTIFACTS_TOOL_OPEN_ID, ARTIFACTS_TOOL_STATE_ID])
	})

	test("catalog projects the artifacts side-panel tab with stable identity", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const panel = catalog.projections.panels.find((p) => p.pluginId === ARTIFACTS_PLUGIN_ID)
		expect(panel).toBeDefined()
		expect(panel?.projectedId).toBe(ARTIFACTS_PANEL_PROJECTED_ID)
		expect(panel?.availability.available).toBe(true)

		const descriptor = catalogPanelToTabDescriptor(panel!)
		expect(descriptor).not.toBeNull()
		expect(descriptor?.id).toBe("artifacts")
		// criterion 6: persistence + telemetry identity survive the cutover.
		expect(descriptor?.persistenceKey).toBe("side-panel.artifacts")
		expect(descriptor?.telemetryNamespace).toBe("firefly.surface.artifacts")
		expect(descriptor?.renderMode).toBe("host-reconciler")
	})

	test("artifacts tools project into the bridge tool surface (OpenCode visibility)", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const descriptor = catalog.descriptors.find((d) => d.normalizedId === ARTIFACTS_PLUGIN_ID)
		const projected = projectBridgeToolDefinitions(descriptor!)
		expect(projected.map((t) => t.id)).toContain(ARTIFACTS_TOOL_OPEN_ID)
		expect(projected.map((t) => t.id)).toContain(ARTIFACTS_TOOL_STATE_ID)
	})
})

describe("criterion 3 — paired tools dispatch with typed envelopes", () => {
	test("artifacts.open opens the side panel", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeArtifactsDeps()
		registerArtifactsHostHandlers(deps)
		const result = await invokePluginTool({
			pluginId: ARTIFACTS_PLUGIN_ID,
			toolId: ARTIFACTS_TOOL_OPEN_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
		expect(opened).toEqual(["artifacts"])
		expect((result.data as { tab: string }).tab).toBe("artifacts")
	})

	test("artifacts.state returns typed surface state", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		registerArtifactsHostHandlers(fakeArtifactsDeps().deps)
		const result = await invokePluginTool({
			pluginId: ARTIFACTS_PLUGIN_ID,
			toolId: ARTIFACTS_TOOL_STATE_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
	...
	test("unknown tool and invalid args produce typed failures, not throws", async () => {
		...
		const unknown = await invokePluginTool({
			pluginId: ARTIFACTS_PLUGIN_ID,
			toolId: "plugin.firefly.built-in.surface.artifacts.ghost",
			args: {},
		})
	...
	test("open-artifacts / toggle-artifacts commands dispatch through the V2 command path", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeArtifactsDeps()
...
describe("criterion 4 — disable/enable round-trip via host lifecycle", () => {
	test("disable flips catalog status + projection availability; tools refuse; enable restores", async () => {
		...
		setPluginEnabled(ARTIFACTS_PLUGIN_ID, false)
		const disabledCatalog = getPluginCatalog()
		const entry = disabledCatalog.entries.find((e) => e.pluginId === ARTIFACTS_PLUGIN_ID)
		expect(entry?.status).toBe("disabled")
		const panel = disabledCatalog.projections.panels.find((p) => p.pluginId === ARTIFACTS_PLUGIN_ID)
		...
		const commands = disabledCatalog.projections.commands.filter(
			(c) => c.pluginId === ARTIFACTS_PLUGIN_ID,
		)
		...
		setPluginEnabled(ARTIFACTS_PLUGIN_ID, true)
		const restored = getPluginCatalog()
		expect(
			restored.projections.panels.find((p) => p.pluginId === ARTIFACTS_PLUGIN_ID)?.availability
				.available,
		).toBe(true)
	})
})
...
describe("criterion 5 — UI-crash quarantine drill", () => {
	test("3 panel crashes quarantine the plugin; release restores it", async () => {
		...
		reportPluginPanelCrash(ARTIFACTS_PLUGIN_ID, "drill crash 1")
		reportPluginPanelCrash(ARTIFACTS_PLUGIN_ID, "drill crash 2")
		const third = reportPluginPanelCrash(ARTIFACTS_PLUGIN_ID, "drill crash 3")
		expect(third.quarantined).toBe(true)

		const catalog = getPluginCatalog()
		const entry = catalog.entries.find((e) => e.pluginId === ARTIFACTS_PLUGIN_ID)
		...
		releasePluginQuarantine(ARTIFACTS_PLUGIN_ID, "drill release")
		const restored = getPluginCatalog()
		expect(restored.entries.find((e) => e.pluginId === ARTIFACTS_PLUGIN_ID)?.status).toBe("validated")
	})
})
...
describe("criterion 7 — legacy flag migration", () => {
	...
	test("flag=false migrates to a host-side disable, once", async () => {
		const { io, enabledCalls, store } = migrationIo({ "elf:artifactsSurfaceEnabled": "false" })
		const first = await migrateSurfaceFlag(migration, io)
		expect(first.action).toBe("migrated-disabled")
		expect(enabledCalls).toEqual([{ pluginId: ARTIFACTS_PLUGIN_ID, enabled: false }])
		expect(store.has("elf:artifactsSurfaceEnabled")).toBe(false)
	...
	test("flag=true needs no host write (enabled is the default)", async () => {
		const { io, enabledCalls } = migrationIo({ "elf:artifactsSurfaceEnabled": "true" })
