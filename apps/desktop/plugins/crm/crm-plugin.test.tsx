/**
 * Slice-proof tests for the CRM migration (plan §6 criteria):
 *  1. registry row deleted — grep-proof on the registry source;
 *  2. the CRM tab derives solely from the catalog projection;
...
function fakeCrmDeps() {
	const opened: string[] = []
	let enabled = true
	return {
		opened,
		deps: {
			openSidePanel: async (tab: "crm") => {
				opened.push(tab)
			},
			getSidePanelState: () => ({
				open: true,
				activeTab: "crm",
				availableTabs: ["review", "crm", "files"],
			}),
...
describe("criterion 1 — registry row deleted (grep-proof)", () => {
	test("firefly-surface-registry.tsx has no crm surface row", () => {
		const source = fs.readFileSync(REGISTRY_SOURCE, "utf8")
		expect(source).not.toContain('id: "crm"')
		expect(source).not.toContain("CrmPanel")
		expect(source).not.toContain("crmSurfaceEnabledAtom")
	})

	test("the legacy panel file is gone from components/side-panel", () => {
		const legacyPath = path.join(
			import.meta.dir,
			"../../src/renderer/components/side-panel/crm-panel.tsx",
		)
		expect(fs.existsSync(legacyPath)).toBe(false)
	})

	test("feature-flags.ts has no crmSurfaceEnabledAtom", () => {
		const source = fs.readFileSync(FEATURE_FLAGS_SOURCE, "utf8")
		expect(source).not.toContain("crmSurfaceEnabledAtom =")
		expect(source).not.toContain("toggleCrmSurfaceAtom =")
	})
})

describe("criterion 2 — CRM serves from the catalog projection", () => {
	test("manifest parses and derives a descriptor on the canonical path", () => {
		const manifest = parsePluginManifest(crmPluginManifest)
		const descriptor = derivePluginDescriptor(manifest, { appVersion: "0.11.0" })
		expect(descriptor.normalizedId).toBe(CRM_PLUGIN_ID)
		expect(descriptor.panels).toHaveLength(1)
		expect(descriptor.tools.map((t) => t.id)).toEqual([CRM_TOOL_OPEN_ID, CRM_TOOL_STATE_ID])
	})

	test("catalog projects the crm side-panel tab with stable identity", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const panel = catalog.projections.panels.find((p) => p.pluginId === CRM_PLUGIN_ID)
		expect(panel).toBeDefined()
		expect(panel?.projectedId).toBe(CRM_PANEL_PROJECTED_ID)
		expect(panel?.availability.available).toBe(true)

		const descriptor = catalogPanelToTabDescriptor(panel!)
		expect(descriptor).not.toBeNull()
		expect(descriptor?.id).toBe("crm")
		// criterion 6: persistence + telemetry identity survive the cutover.
		expect(descriptor?.persistenceKey).toBe("side-panel.crm")
		expect(descriptor?.telemetryNamespace).toBe("firefly.surface.crm")
		expect(descriptor?.renderMode).toBe("host-reconciler")
	})

	test("crm tools project into the bridge tool surface (OpenCode visibility)", () => {
		_resetPluginAuthorityForTests()
		const catalog = getPluginCatalog()
		const descriptor = catalog.descriptors.find((d) => d.normalizedId === CRM_PLUGIN_ID)
		const projected = projectBridgeToolDefinitions(descriptor!)
		expect(projected.map((t) => t.id)).toContain(CRM_TOOL_OPEN_ID)
		expect(projected.map((t) => t.id)).toContain(CRM_TOOL_STATE_ID)
	})
})

describe("criterion 3 — paired tools dispatch with typed envelopes", () => {
	test("crm.open opens the side panel", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeCrmDeps()
		registerCrmHostHandlers(deps)
		const result = await invokePluginTool({
			pluginId: CRM_PLUGIN_ID,
			toolId: CRM_TOOL_OPEN_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
		expect(opened).toEqual(["crm"])
		expect((result.data as { tab: string }).tab).toBe("crm")
	})

	test("crm.state returns typed surface state", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		registerCrmHostHandlers(fakeCrmDeps().deps)
		const result = await invokePluginTool({
			pluginId: CRM_PLUGIN_ID,
			toolId: CRM_TOOL_STATE_ID,
			args: {},
		})
		expect(result.status).toBe("completed")
	...
	test("unknown tool and invalid args produce typed failures, not throws", async () => {
		...
		const unknown = await invokePluginTool({
			pluginId: CRM_PLUGIN_ID,
			toolId: "plugin.firefly.built-in.surface.crm.ghost",
			args: {},
		})
	...
	test("open-crm / toggle-crm commands dispatch through the V2 command path", async () => {
		_resetPluginAuthorityForTests()
		_resetHostToolsForTests()
		_resetHostCommandsForTests()
		const { deps, opened } = fakeCrmDeps()
...
describe("criterion 4 — disable/enable round-trip via host lifecycle", () => {
	test("disable flips catalog status + projection availability; tools refuse; enable restores", async () => {
		...
		setPluginEnabled(CRM_PLUGIN_ID, false)
		const disabledCatalog = getPluginCatalog()
		const entry = disabledCatalog.entries.find((e) => e.pluginId === CRM_PLUGIN_ID)
		expect(entry?.status).toBe("disabled")
		const panel = disabledCatalog.projections.panels.find((p) => p.pluginId === CRM_PLUGIN_ID)
		...
		setPluginEnabled(CRM_PLUGIN_ID, true)
		const restored = getPluginCatalog()
		expect(
			restored.projections.panels.find((p) => p.pluginId === CRM_PLUGIN_ID)?.availability
				.available,
		).toBe(true)
	})
})
...
describe("criterion 5 — UI-crash quarantine drill", () => {
	test("3 panel crashes quarantine the plugin; release restores it", async () => {
		...
		reportPluginPanelCrash(CRM_PLUGIN_ID, "drill crash 1")
		reportPluginPanelCrash(CRM_PLUGIN_ID, "drill crash 2")
		const third = reportPluginPanelCrash(CRM_PLUGIN_ID, "drill crash 3")
		expect(third.quarantined).toBe(true)

		const catalog = getPluginCatalog()
		const entry = catalog.entries.find((e) => e.pluginId === CRM_PLUGIN_ID)
		...
		releasePluginQuarantine(CRM_PLUGIN_ID, "drill release")
		const restored = getPluginCatalog()
		expect(restored.entries.find((e) => e.pluginId === CRM_PLUGIN_ID)?.status).toBe("validated")
	})
})
...
describe("criterion 7 — legacy flag migration", () => {
	...
	test("flag=false migrates to a host-side disable, once", async () => {
		const { io, enabledCalls, store } = migrationIo({ "elf:crmSurfaceEnabled": "false" })
		const first = await migrateSurfaceFlag(migration, io)
		expect(first.action).toBe("migrated-disabled")
		expect(enabledCalls).toEqual([{ pluginId: CRM_PLUGIN_ID, enabled: false }])
		expect(store.has("elf:crmSurfaceEnabled")).toBe(false)
	...
	test("flag=true needs no host write (enabled is the default)", async () => {
		const { io, enabledCalls } = migrationIo({ "elf:crmSurfaceEnabled": "true" })
	...
	test("migration row present in SURFACE_FLAG_MIGRATIONS", async () => {
		const { SURFACE_FLAG_MIGRATIONS } = await import("../../src/renderer/firefly-plugin-flag-migration")
		const row = SURFACE_FLAG_MIGRATIONS.find((m) => m.pluginId === CRM_PLUGIN_ID)
		expect(row).toBeDefined()
		expect(row?.legacyStorageKey).toBe("elf:crmSurfaceEnabled")
	})
})
