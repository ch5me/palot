...
const Panel = ({ title, detail }: { title: string; detail: string }) => (
	<div className="flex h-full flex-col justify-between bg-[radial-gradient(circle_at_top_left,rgba(20,184,166,.24),transparent_38%),linear-gradient(135deg,rgba(15,23,42,.96),rgba(30,41,59,.9))] p-8 text-white">
		<div>
			<div className="text-sm font-medium uppercase tracking-[0.18em] text-white/55">Coverage lane</div>
...
const items = [
	{
		...
		content: <Panel title="Discover" detail="Start from local source, not imported package assumptions." />,
	},
	{
		value: "compose",
		label: "Compose",
		description: "Build a realistic state with controls and dense UI rhythm.",
		content: <Panel title="Compose" detail="Use representative Palot data and expose meaningful interactive states." />,
	},
	{
		value: "prove",
		label: "Prove",
		description: "Render screenshot targets and verify CH5 mapping.",
		content: <Panel title="Prove" detail="Only check tracker boxes after Storybook and CH5 both see the story." />,
	},
]
