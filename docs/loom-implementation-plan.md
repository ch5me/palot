...
> `docs/loom-build-prompts/` (the *how*, one prompt per wave),
> `docs/loom-progress.md` (the *status*).
...
The build prompts in `docs/loom-build-prompts/` are scoped to one wave each.
...
- The 18 surface ids appear in 7 places today. Consolidate to one
  ...
  `panels` family. See Appendix A in `docs/loom-alignment-assessment.md`.
...
| Risk | Mitigation |
|---|---|
| Wave 0 is not strictly Loom and may be deprioritized. | Frame it as a prerequisite, not a Loom wave. Pin in `docs/loom-progress.md` as a blocker for Wave 1. |
...
1. `docs/loom-progress.md` is updated with the wave's status. <!-- oc:id=item_ae -->
1. A changeset (`bun changeset`) is added if any user-facing change is <!-- oc:id=item_af -->
   shipped.
1. A PR is opened; the PR description cites this plan and the relevant <!-- oc:id=item_ag -->
   build prompt from `docs/loom-build-prompts/`.
...
The exact prompt is in `docs/loom-build-prompts/wave-01-typed-registry.md`.
